...
public class CameraMove : MonoBehaviour
{
    ...
    void Update()
    {
        if (Catapult.catapultstate == CatapultState.Idle && GameController.CurrentGameState == GameState.Playing)
        {
            //drag start
            ...
            else if (Input.GetMouseButton(2) && Time.time - timeDragStarted > 0.05f)
            {
                ...
                float newX = Mathf.Clamp(transform.position.x + deltaX, 0, 30);
                float newY = Mathf.Clamp(transform.position.y + deltaY, 0, 3f);
                //move camera
